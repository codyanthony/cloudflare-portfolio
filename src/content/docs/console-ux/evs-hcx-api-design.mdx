...
description: Advocating for extensible architecture aligned with AWS API design standards during EVS HCX feature development
---

import { Card, CardGrid, Aside } from "@astrojs/starlight/components";

This project demonstrates upstream product influence during EVS HCX feature API design. As a customer experience advocate in API design reviews, I surfaced parallel VPC development work that could reduce implementation overhead, raised concerns about backward compatibility and long-term maintainability, and advocated for alignment with AWS API design standards. These discussions influenced the team's decision to adopt an extensible architecture that supports current needs while enabling future capabilities.

<Aside type="tip" title="At a Glance">

- **Challenge:** EVS planning API design for HCX elastic IP association feature
- **My Role:** Customer Experience Advocate in API Design Reviews
- **Timeline:** Q1-Q3 2025 (Collaboration → Influence → Launch)
...
Initial architectural discussions focused on a single-purpose API design scoped specifically to the HCX feature. This reflected delivery timeline priorities and limited awareness of parallel VPC development work that could inform the design. AWS API design standards emphasize extensible architectures from v1 because public APIs must maintain backward compatibility indefinitely—they can never be removed once shipped. Surfacing this context alongside the parallel VPC work created an opportunity to align the HCX API design with these long-term maintainability principles.
...
In Q1 2025, I established daily 1:1 syncs with EVS PMs and engineers and integrated documentation into engineering workflows using shared project management tools. These collaboration mechanisms created visibility into upcoming architectural decisions and positioned me to contribute meaningfully during Q2 HCX API design discussions.
